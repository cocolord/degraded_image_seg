...
class VIT_MLA_ConvFuse_UnetLike(nn.Module):
    ...
    def __init__(self, model_name='vit_large_patch16_384', img_size=384, patch_size=16, in_chans=3, embed_dim=1024, depth=24,
        ...
        super(VIT_MLA_ConvFuse_UnetLike, self).__init__(**kwargs)
...
@BACKBONES.register_module()
class Conv_Fuse_Unetlike(nn.Module):
    def __init__(self):
        super(Conv_Fuse_Unetlike, self).__init__()
    ...
    def forward(self, x):
        b,c,h,w = x.size() # 2,3,768,768
        l1_fm = self.layer1(x) # 64,384,384
        l2_fm = self.layer2(l1_fm) # 128,192,192
        l3_fm = self.layer3(l2_fm) # 256,96,96
        l4_fm = self.layer4(l3_fm) # 512, 48, 48
        agg_fm = self.layer4_enc(l4_fm)
        agg_fm = torch.cat((l3_fm,agg_fm),dim=1)
        agg_fm = self.layer3_enc(agg_fm)
